...
- (void)setFaceInfo:(NSDictionary *)faceInfo{
    ...
    float valueScore = [value floatValue];
    [detailStr appendFormat:@"\n颜值（女生打分）: %.2lf", valueScore];
    value = att[@"beauty"][@"male_score"];
    valueScore = [value floatValue];
    [detailStr appendFormat:@"\n颜值（男生打分）: %.2lf", valueScore];
    
    //AJ
    value = att[@"skinstatus"][@"dark_circle"];
    [detailStr appendFormat:@"\n\n黑眼圈: %@",value];
...
- (void)handleImage:(UIImage *)image{
    ...
    //需要获取的属性
    NSArray *att = @[@"gender",@"age",@"beauty", @"skinstatus", @"headpose",@"smiling",@"blur",@"eyestatus",@"emotion",@"facequality",@"ethnicity"];
